...
target_path='~/A_Projects/NanostringR/data/FUNKIS/RCCData/'
sample_sheet<-read.csv('~/A_Projects/NanostringR/data/FUNKIS/samp_sheet_no_PBMC.csv')
...
nsn_results<-QC_plus_nSolver_processing(guide_vect=group_names_vector,
                                          ref_name=reference_name,
                                          controls_only_norm = TRUE,
                                          remove_failed_HK=FALSE)
#Extract QC data and feature data
QC_data <- nsn_results$QCdata
```
...
```{r PCA}
pca_plot <- plot_PCA(normalized_counts = nsn_results$normalized_counts,
                     QCdata = QC_data)
pca_plot
```
```{r save_PCA, include=FALSE}
ggsave(filename = paste0(res_name, 'summary_PCA.png'), plot = pca_plot,
...
normalized_counts<-nsn_results$normalized_counts
